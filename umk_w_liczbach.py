...
if sekcja == 'Strona główna':
    ...
    with k1:
        ...
        fig.update_layout(title="Grupa badawcza",legend=dict(x=0,y=1.2),margin=dict(t=80, b=100, l=0, r=100),plot_bgcolor='white',font=dict(family='Lato',size=18,color="Black"),separators=',',showlegend=False)
        st.plotly_chart(fig,use_container_width=True)
    with k2:
        ...
        fig.update_traces(textfont=dict( size=14),textinfo='value+percent',marker=dict( colors=['rgb(0,165,80)','rgb(170,210,60)','rgb(250,20,20)','rgb(255,130,30)']),direction ='clockwise',hovertemplate = '%{label}'+"<extra></extra>")
        fig.update_layout(title="Grupa badawczo-dydaktyczna",legend=dict(x=0,y=1.2),margin=dict(t=80, b=100, l=0, r=100),plot_bgcolor='white',font=dict(family='Lato',size=18,color="Black"),separators=',',showlegend=False)
        st.plotly_chart(fig,use_container_width=True)     
    with k3:
    ...
    if (li == 'Kwota') and (roki in [2019,2020,2021]) :		       
	    ...
	    fig.update_xaxes(title='Kwota wnioskowana[zł]',range=[0,y[::-1][0]+y[::-1][0]/30])
	    fig.update_yaxes(title='Wydział')
	    ...
	    fig.update_xaxes(title='Liczba wniosków',range=[0,y[::-1][0]+1])
	    fig.update_yaxes(title='Wydział')
    ...
    if li == 'Kwota':
        ...
        fig.update_xaxes(title='Kwota przyznana[zł]',range=[0,y[::-1][0]+y[::-1][0]])
        fig.update_yaxes(title='Wydział')
        ...
        fig.update_xaxes(title='Liczba grantów',range=[0,y[::-1][0]+1])
        fig.update_yaxes(title='Wydział')
    ...
    if (li1 == 'Kwota') and (roki1 in [2015,2016,2017,2018,2019,2020,2021]):
        ...
        fig.update_traces(marker_color=barwa3,marker_line_color='black',marker_line_width=1.5,
                          textposition='outside',texttemplate = "<b>%{x:,t}",hovertemplate = 'Kwota przyznanych grantów: %{x:,}zł'+"<extra></extra>")
        fig.update_xaxes(title='Kwota przyznana[zł]',range=[0,y[::-1][0]+y[::-1][0]/30])
        fig.update_yaxes(title='Wydział')
