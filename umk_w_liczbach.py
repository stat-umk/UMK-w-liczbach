...
if sekcja == 'Studenci':
    ...
    if gz == 'Włącz':
	    fig5 = px.line(DF19,x='Rok',y='Liczba',color_discrete_sequence=['rgb(0,80,170)'],markers=True).update_traces(hovertemplate = 'Średnia liczba studentów: %{y:}',textfont=dict( size=14),textposition="top left",texttemplate = "%{y:.2f}").update_yaxes(tickformat=",").update_layout(plot_bgcolor='white',font=dict(family='Lato',size=18,color="Black"),separators=',')
    ...
    with d1:
    	...
    	fig.add_trace(go.Bar(x=y,y=x,orientation='h',text=y,marker_color=barwa4,
                        textfont=dict( size=12,color='black'),hover_name=x,
                      textposition='outside',texttemplate = "<b>Przyznane-%{x:}").update_traces(marker_line_color='black',marker_line_width=1.5,hovertemplate = 'Wnioski złożone: %{y:}'))
	    
    	fig.add_trace(go.Bar(x=y1,y=x1,orientation='h',text=y1,marker_color=barwa5,hover_name=x1,
                        textfont=dict( size=12,color='black'),
                      textposition='outside',texttemplate = "<b>Złożone-%{x:}").update_traces(marker_line_color='black',marker_line_width=1.5,hovertemplate = 'Wnioski złożone: %{y1:}'))
