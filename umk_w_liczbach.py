...
if sekcja == 'Strona główna':
    ...
    with k3 :
        ...
        fig.update_layout(title="<b>Grupa dydaktyczna</b>",legend=dict(x=0,y=0),margin=dict(t=80, b=0, l=0, r=100),plot_bgcolor='white',font=dict(family='Lato',size=20,color="Black"),separators=',',title_x=0.1,title_y=0.95,showlegend=False)
        st.plotly_chart(fig,use_container_width=True)
    ...
    if (li == 'Kwota'):
        ...
        fig.update_layout(xaxis=dict(showline=False,showgrid=True,showticklabels=True,linewidth=2,linecolor='black',gridwidth=1,gridcolor='gray',mirror=True),title='<b>Wnioski złożone',title_x=0.5,
        					height=800,width=1600,plot_bgcolor='white',margin=dict(t=200, b=0, l=180, r=100),font=dict(family='Lato',size=14,color="Black"),showlegend=True,legend_orientation='h',legend_x=-0.1,legend_yanchor='top',legend_y=1.1,
