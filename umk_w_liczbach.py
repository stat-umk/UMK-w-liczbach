...
if sekcja == 'Strona główna':
    ...
    st.plotly_chart(px.bar(DF21[DF21['Rodzaj']==pr],x='Rok',y='Liczba',text='Liczba',color='Jednostka',width=1400,height=500,color_discrete_sequence=['rgb(255,205,0)','rgb(250,20,20)']).update_traces(texttemplate="%{y:}",hovertemplate = 'Liczba pracowników: %{y:}',
	textposition='inside',textfont=dict( size=14))
    ...
    st.plotly_chart(px.line(DF28[DF28['Kategoria'].isin(['Adiunkci','Profesorowie','Ogółem','Asystenci i lektorzy'])].sort_values('Kategoria',key=lambda column: column.map(lambda e: pr_cy2.index(e)),x='Rok',y='Wynagrodzenie',color='Kategoria',width=1400,height=500,markers=True,color_discrete_sequence=list(map(lambda x: pr_cy[x],wydz318)))
