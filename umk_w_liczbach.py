...
if sekcja == 'Strona główna':
    ...
    st.plotly_chart(px.line(DF28[DF28['Kategoria'].isin(wydz318)].sort_values(by='Kategoria',key=lambda column: column.map(lambda e: pr_cy2.index(e))),x='Rok',y='Wynagrodzenie',color='Kategoria',width=1400,height=500,markers=True,color_discrete_sequence=list(map(lambda x: pr_cy[x],sorted(wydz318,key=lambda x: pr_cy1[x]))))
