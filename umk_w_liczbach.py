...
if sekcja == 'Strona główna':
    ...
    st.plotly_chart(px.bar(DF21[DF21['Rodzaj']==pr],x='Rok',y='Liczba',customdata=text=DF21[DF21['Rodzaj']==pr].groupby('Rok')['Liczba'].agg(np.sum),color='Jednostka',width=1400,height=500,color_discrete_sequence=['rgb(255,205,0)','rgb(250,20,20)']).update_traces(texttemplate="%{y:}",hovertemplate = 'Liczba ogółem: %{customdata}'+"<extra></extra>",
	textposition='inside',textfont=dict( size=14))
