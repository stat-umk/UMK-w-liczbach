...
DF8 = pd.read_excel(io='Studenci.xlsx',engine='openpyxl',sheet_name='N-wni',dtype={'Rok':str})
...
if sekcja == 'Studenci':
    ...
    st.plotly_chart(px.bar(DF7,x='Rok',y='Liczba',width=1050,height=400).update_traces(marker_color='rgb(0,80,170)',texttemplate="%{y:}",
    ...
    st.plotly_chart(px.line(DF8,x='Rok',y='Odsetek',color = 'Rodzaj',width=1250,height=400,text='Odsetek',color_discrete_sequence=['rgb(0,80,170)','rgb(0,200,255)','red','rgb(255,50,80)'])
		    .update_traces(marker_color=('rgb(0,80,170)'),textposition="top right",texttemplate = "%{y:.2f}%")
	.update_xaxes(title_font=dict(size=12), title='Rok',range=[0, None]).update_yaxes(title_font=dict(size=12),title = 'Odsetek osób niepełnosprawnych',tickformat=",")
