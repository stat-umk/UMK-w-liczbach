...
if sekcja == 'Studenci':
    ...
    if kat34 == 'Studia stacjonarne':
        ...
        st.plotly_chart(px.bar(DF11[DF11['Wydział']==wydzial34],x='Rok',y='Liczba',width=1500,height=500)
			.update_traces(marker_color=kolwyd[wydzial34],texttemplate="%{y:}",textposition='inside')
			.update_xaxes(dtick=1)
			.update_yaxes(range=[0,np.max(DF11[DF11['Wydział']==wydzial34]['Liczba'])+(1/10)*(np.max(DF11[DF11['Wydział']==wydzial34]['Liczba'])+1)])
			.update_layout(font_family='Lato'))
	...
	.update_xaxes(zeroline=True, zerolinewidth=10, zerolinecolor='LightPink',title_font=dict(size=12), title='Rok',range=[2011.95,2022],dtick=1)
		    .update_yaxes(title_font=dict(size=12),title = 'Odsetek osób zagranicznych',tickformat=",",range=[-0.05, 5.5])
		    .update_layout(font_family='Lato',separators=','))
    ...
    fig4 = px.bar(DF15[(DF15['Wydział'].isin([wydz11,wydz22]))],x='Rok',y='Liczba',barmode = 'group', color='Wydział',width=1500,height=500,color_discrete_map={wydz11: kolwyd[wydz11],wydz22: kolwyd[wydz22]},pattern_shape="Wydział").update_traces(texttemplate="%{y:}",textposition='inside').update_xaxes(dtick=1).update_layout(font_family='Lato')
    fig5 = px.line(DF19,x='Rok',y='Liczba',text='Liczba',color_discrete_sequence=['rgb(0,80,170)']).update_traces(textposition="top right",texttemplate = "%{y:.2f}").update_yaxes(tickformat=",").update_layout(font_family='Lato',separators=',')
    ...
    st.plotly_chart(px.line(DF15[(DF15['Wydział'].isin(wydz1))],x='Rok',y='Liczba',color='Wydział',width=1400,height=500,symbol='Wydział',markers=True,text='Liczba',color_discrete_sequence=sorted(list(map(lambda x: kolwyd[x],wydz1))))
