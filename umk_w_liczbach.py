...
if sekcja == 'Nauczyciele akademiccy i administracja':
    ...
    fig = px.line(DF5[DF5['Jednostka Organizacyjna'].isin([wydział,wydział1])],x='Rok',
		  ...
		  textposition="top right").update_xaxes(autorange="reversed").update_layout(font_family='Lato').add_annotation(x=0, y=int(DF5[(DF5['Rok']=='2021') & (DF5['Jednostka Organizacyjna']==wydział)]['Liczba nauczycieli akademickich'])+5,
            text=wydział,showarrow=True,font=dict(size=16)).add_annotation(x=0, y=int(DF5[(DF5['Rok']=='2021') & (DF5['Jednostka Organizacyjna']==wydział1)]['Liczba nauczycieli akademickich'])-5,
            text=wydział1,showarrow=True,font=dict(size=16))
    st.plotly_chart(fig)
