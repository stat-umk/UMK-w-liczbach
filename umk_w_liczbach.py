...
if sekcja == 'Strona główna':
    ...
    if sekcja3 == 'Granty Narodowego Centrum Nauki':
        ...
        if (li == 'Kwota'):
            ...
            kw1 = pd.DataFrame(DF6[DF6['Rok']==roki].groupby('Jednostka')['Kwota przyznana[zł]'].agg(np.sum)).sort_values(by='Kwota przyznana[zł]')[::-1].append(pd.Series({'Jednostka':' ','Kwota przyznana[zł]':-100,'kolor':'white'},name=17))
            x1 = kw1.index[::-1]
