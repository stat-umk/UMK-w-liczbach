...
if sekcja == 'Nauczyciele akademiccy i administracja':
    ...
    fig = px.line(DF5[DF5['Jednostka Organizacyjna'].isin([wydział,wydział1])],x='Rok',
		  ...
		  textposition="top right").update_xaxes(autorange="reversed").update_layout(font_family='Lato').add_annotation(x=1, y=DF5[(DF5['Rok']=='2021') & (DF5['Jednostka Organizacyjna']==wydział)]['Liczba nauczycieli akademickich'],
            text='dads')
    st.plotly_chart(fig)
