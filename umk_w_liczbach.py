...
if sekcja == 'Strona główna':
    ...
    if sekcja2 == 'Pracownicy':
        ...
        if gz == 'Włącz':
	        fig5 = px.line(DF25,x='Rok',y='Liczba',color_discrete_sequence=['rgb(0,80,170)'],markers=True).update_traces(textfont=dict( size=14),textposition="top left",texttemplate = "%{y:.2f}",hovertemplate = 'Średnia liczba nauczycieli akademickich: <b>%{y:,.2f}</b>').update_yaxes(rangemode='tozero',tickformat=",").update_layout(plot_bgcolor='white',font=dict(family='Lato',size=18,color="Black"),separators=',')
